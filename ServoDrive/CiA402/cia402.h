...
/*
 * Determine the Controlword for the normal CiA-402 enable sequence.
 *
 * Returns true when the current state is part of the normal enable path.
 *
 *      Not Ready to Switch On -> wait safely with 0x0000
 *      Switch On Disabled     -> 0x0006
 *      Ready to Switch On     -> 0x0007
 *      Switched On            -> 0x000F
 *      Operation Enabled      -> 0x000F
 *
 * Returns false for states that must NOT be automatically enabled:
 *
 *      Quick Stop Active
